...
from timm.data import create_dataset, create_loader, resolve_data_config, Mixup, FastCollateMixup, AugMixDataset, create_transform
from timm.layers import convert_splitbn_model, convert_sync_batchnorm, set_fast_norm
from timm.loss import JsdCrossEntropy, SoftTargetCrossEntropy, BinaryCrossEntropy, SafeASLSingleLabel, LabelSmoothingCrossEntropy, FocalLoss, CBCE, CBFocal
from timm.models import create_model, safe_model_name, resume_checkpoint, load_checkpoint, model_parameters
from timm.optim import create_optimizer_v2, optimizer_kwargs
from timm.scheduler import create_scheduler_v2, scheduler_kwargs
from timm.utils import ApexScaler, NativeScaler
from dataset_reefnet_v2 import ReefNetDataSet
...
parser.add_argument('--csv_file', default='', type=str,
                            help='csv file path')
parser.add_argument('--label_column', default='Experimental_label', type=str,
                            help='label column')
parser.add_argument('--path_column', default='patch_path', type=str,
                            help='path column')
...
# Loss ablation ---------------------------------------------------------------
loss_grp = parser.add_argument_group('Loss ablation')
loss_grp.add_argument('--loss', default='ce',
                      choices=['ce', 'lsce', 'asl', 'focal', 'cbce', 'cbfocal'],
                      help='which loss function to use')
loss_grp.add_argument('--gamma', type=float, default=2.0,
                      help='gamma for focal / cbfocal')
loss_grp.add_argument('--alpha', type=float, default=0.25,
                      help='alpha for focal (None disables class weighting)')
loss_grp.add_argument('--beta',  type=float, default=0.999,
                      help='beta for class-balanced losses')
...
def main():
    ...
    if args.dataset == 'reefnet_csv':
        transform_train = create_transform(
            input_size=args.input_size,
            is_training=True,
        )
        dataset_train = ReefNetDataSet(
        ...
        # ---- class counts for CB losses --------------------------------------------
        if hasattr(dataset_train, 'class_counts'):          # ReefNetDataSet exposes it
            samples_per_cls = torch.tensor(dataset_train.class_counts, dtype=torch.float)
        elif hasattr(dataset_train, 'targets'):             # ImageFolder etc.
            import numpy as np
            samples_per_cls = torch.tensor(
                np.bincount(dataset_train.targets, minlength=args.num_classes), dtype=torch.float)
        else:
            samples_per_cls = None      # will raise later if cb-loss is requested
        transform_val = create_transform(
            input_size=args.input_size,
            is_training=False,
        )
        dataset_eval = ReefNetDataSet(
            csv_file=args.csv_file,
            split='val',
            label_column=args.label_column,
            path_column=args.path_column,
    ...
    #     if args.jsd_loss:
    #         print("\n" + "="*50)
    #         print("Using JsdCrossEntropy loss")
    #         print("="*50 + "\n")
    ...
    #         if args.bce_loss:
    #             print("\n" + "="*50)
    #             print("Using BinaryCrossEntropy loss with mixup active")
    #             print("="*50 + "\n")
    ...
    #         else:
    #             print("\n" + "="*50)
    #             print("Using SoftTargetCrossEntropy loss with mixup active")
    #             print("="*50 + "\n")
    #             train_loss_fn = SoftTargetCrossEntropy()
    #     elif args.smoothing:
    #         if args.bce_loss:
    #             print("\n" + "="*50)
    #             print("Using BinaryCrossEntropy loss with smoothing")
    #             print("="*50 + "\n")
    #             train_loss_fn = BinaryCrossEntropy(
    ...
    if args.loss == 'ce':
        ...
        train_loss_fn = LabelSmoothingCrossEntropy(smoothing=args.smoothing or 0.1)
        _logger.info("\n" + "="*50 + "\nUsing LabelSmoothingCrossEntropy\n" + "="*50)
    elif args.loss == 'asl':
        train_loss_fn = SafeASLSingleLabel()
        _logger.info("\n" + "="*50 + "\nUsing AsymmetricLossSingleLabel\n" + "="*50)
    elif args.loss == 'focal':
        train_loss_fn = FocalLoss(gamma=args.gamma, alpha=args.alpha)
        _logger.info("\n" + "="*50 + "\nUsing FocalLoss\n" + "="*50)
    elif args.loss == 'cbce':
        assert samples_per_cls is not None, 'dataset must expose class counts for cbce'
        train_loss_fn = CBCE(samples_per_cls, beta=args.beta)
        _logger.info("\n" + "="*50 + "\nUsing Class-Balanced CrossEntropyLoss (CBCE)\n" + "="*50)
    elif args.loss == 'cbfocal':
        assert samples_per_cls is not None, 'dataset must expose class counts for cbfocal'
        train_loss_fn = CBFocal(samples_per_cls, beta=args.beta, gamma=args.gamma)
        _logger.info("\n" + "="*50 + "\nUsing Class-Balanced FocalLoss (CBFocal)\n" + "="*50)
    else:
        train_loss_fn = nn.CrossEntropyLoss()

#!/bin/bash
#SBATCH --job-name=vit-scratch-reefnet-cbfocal
#SBATCH --nodes=1
#SBATCH --gres=gpu:a100:4
#SBATCH --cpus-per-gpu=10
#SBATCH --mem=196G
#SBATCH --time=23:30:00
#SBATCH --output=output/ViT-Scratch_CBFocal/logs/%j-%x.out
...
torchrun --nproc-per-node=$GPUS_PER_NODE --master_port=$PORT train.py \
  ...
  --batch-size 128 \
  --epochs 100 \
  --loss cbfocal --beta 0.999 --gamma 2 \
  --amp \
  --channels-last \
  ...
  --clip-grad 1.0 \
  --workers 10 \
  --output output/ViT-Scratch_CBFocal/output/